...
Countries_1990to2019 <- read.csv('C:\\Users\\86177\\Desktop\\肝硬化\\补充\\dalyseapc.csv', header=T) 
load('C:\\Users\\86177\\Desktop\\肝硬化\\map\\GBD_maps.RData')


#3.大地图数据清洗_Prevalence_ASR_2019
##3.1 数据筛选（目的：最终选出204个location的val)####
Prevalence2019_ASR <- Countries_1990to2019 %>%
  select(location_name, location_id, val,measure_name,metric_name,age_name) %>%
...
pal <- tmaptools::get_brewer_pal(palette = "Blues",n = length(breaks_labels)) 
pal <- rev(pal)

##3.3 整理画图数据 (该数据是用于可视化的基础，在本情景下，主要是生成两列变量：1.每个国家的地理边界经纬度信息；2.每个国家的色阶labels)####
Prevalence2019_ASR_map <- left_join(Prevalence2019_ASR, world_GBD, by = c('location_id' = 'Location.ID')) %>%
  mutate(val2 = cut(val, 
                    breaks = breaks,
...
Prevalence2019_ASR_map_plot <- ggplot(data = Prevalence2019_ASR_map,aes(fill = val2)) +
  ...
  theme_void()+
  labs(x="", y="")+
  guides(fill = guide_legend(title='EAPC of ASDR', ncol =2))+
...
###4.1.1 准备作图数据####
a <- Prevalence2019_ASR_map[Prevalence2019_ASR_map$location_name %in% subregions_shp[['Caribbean and central America']]$Location.Name,]
...
## 4.2 Persian Gulf ####
###4.2.1 准备作图数据
a <- Prevalence2019_ASR_map[Prevalence2019_ASR_map$location_name %in% subregions_shp[['Persian Gulf']]$Location.Name,]
...
a <- Prevalence2019_ASR_map[Prevalence2019_ASR_map$location_name %in% subregions_shp[['Balkan Peninsula']]$Location.Name,]
...
## 4.4 Southeast Asia ####
a <- Prevalence2019_ASR_map[Prevalence2019_ASR_map$location_name %in% subregions_shp[['Southeast Asia']]$Location.Name,]
...
## 4.5.West Africa ####
a <- Prevalence2019_ASR_map[Prevalence2019_ASR_map$location_name %in% subregions_shp[['West Africa']]$Location.Name,]
...
a <- Prevalence2019_ASR_map[Prevalence2019_ASR_map$location_name %in% subregions_shp[['Eastern Mediterranean']]$Location.Name,]
...
## 4.7.Northern Europe ####
a <- Prevalence2019_ASR_map[Prevalence2019_ASR_map$location_name %in% subregions_shp[['Nothern Europe']]$Location.Name,]
...
Prevalence2019_ASR_map_plot  <- Prevalence2019_ASR_map_plot / plot3 + plot_layout(height = c(2,1),widths = c(2,1))


#6.保存

ggsave(Prevalence2019_ASR_map_plot, file = 'C:\\Users\\86177\\Desktop\\肝硬化\\pic\\dalyeapcmap.tiff', units = 'cm', width = 24, height = 17.8)
